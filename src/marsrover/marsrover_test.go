...
func TestForward(t *testing.T) {
	...
	assert.DeepEqual(t, Forward(SouthRover{0, 0}), SouthRover{0, -1})
	assert.DeepEqual(t, Forward(EastRover{1, 1}), EastRover{2, 1})
	assert.DeepEqual(t, Forward(WestRover{0, 0}), WestRover{-1, 0})
}

func TestBackward(t *testing.T) {
	assert.DeepEqual(t, Backward(NorthRover{0, 0}), NorthRover{0, -1})
	assert.DeepEqual(t, Backward(SouthRover{2, 3}), SouthRover{2, 4})
	assert.DeepEqual(t, Backward(EastRover{0, 0}), EastRover{-1, 0})
